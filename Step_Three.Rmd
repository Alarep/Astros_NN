...
# I removed the columns for 4 and 5 bangs as it was very rare either showed up (an occurrence of 5 bangs was present one time in the original data set). Not only did I figure this would cause issues in terms of accuracy but it was also causing issues in the normalization stage as there was not enough values to split between the testing and training sets. So when normalization occured there were not enough "positive" values to go around resulting in columns of NaN's
oneHotBangTrain = oneHotBangTrain[,-c(12, 25:26)]
oneHotBangTest = oneHotBangTest[,-c(12, 25:26)]
```

```{r}
# Normalizing the already one hotted training set
...
# Normalizingthe already one hotted testing set.
...
```{r}
observed = testNN$pitch_category_BR
predictedVals <- predict(bangTrainModel, testNN)
residual = observed - predictedVals
plot(predictedVals, residual)
```

```{r}
tester <- subset(testNN, select = c("final_away_runs", "final_home_runs", "inning", "pitch_type_code_CH", "pitch_type_code_CU", "pitch_type_code_EP","pitch_type_code_FC", "pitch_type_code_FF", "pitch_type_code_FS", "pitch_type_code_FT", "pitch_type_code_KC", "pitch_type_code_SI", "pitch_type_code_SL", "pitch_type_code_UN", "pitch_category_BR", "pitch_category_CH",  "pitch_category_FB", "pitch_category_OT", "has_bangs_n", "has_bangs_y", "numOfBangs_1B", "numOfBangs_2B", "numOfBangs_3B", "numOfBangs_0"))

nnResults <- neuralnet::compute(bangTrainModel, tester)
results <- data.frame(actual = testNN$pitch_category_BR, prediction = nnResults$net.result)
